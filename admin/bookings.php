...
function getBookingReportsData($conn) {
    $r = [];
    $res = $conn->query("SELECT COUNT(*) as t FROM bookings WHERE status='pending'");
    $r['pending'] = $res ? $res->fetch_assoc()['t'] : 0;
    $res = $conn->query("SELECT COUNT(*) as t FROM bookings WHERE status='confirmed'");
    $r['confirmed'] = $res ? $res->fetch_assoc()['t'] : 0;
    $res = $conn->query("SELECT COUNT(*) as t FROM bookings WHERE status='cancelled'");
    $r['cancelled'] = $res ? $res->fetch_assoc()['t'] : 0;
    $res = $conn->query("SELECT COALESCE(SUM(total_amount),0) as t FROM bookings WHERE status='confirmed' AND MONTH(created_at)=MONTH(CURDATE()) AND YEAR(created_at)=YEAR(CURDATE())");
    $r['revenueThisMonth'] = $res ? $res->fetch_assoc()['t'] : 0;
    $res = $conn->query("SELECT b.id, b.tour_name, b.booking_date, b.total_amount, b.status, b.number_of_people, CONCAT(u.first_name,' ',u.last_name) as user_name FROM bookings b LEFT JOIN users u ON b.user_id=u.id ORDER BY b.created_at DESC LIMIT 10");
    $r['recent'] = []; if ($res) while ($row=$res->fetch_assoc()) $r['recent'][] = $row;
...
$bookingReports = getBookingReportsData($conn);
...
/* User info cell with proper spacing */
.user-info {
    display: flex;
    align-items: center;
    gap: 14px;
...
/* Recent Bookings table improvements */
.report-table-card td {
    padding: 14px 16px;
}

.report-table-card .user-info-compact {
    display: flex;
    align-items: center;
    gap: 10px;
}

.report-table-card .user-avatar-small {
    width: 34px;
    height: 34px;
    border-radius: 50%;
    background: linear-gradient(135deg, #667eea, #764ba2);
...
<div class="reports-section-title"><span class="material-icons-outlined">assessment</span>Bookings Reports Overview</div>
...
<div class="rsc-header"><div class="rsc-label"><span class="material-icons-outlined">pending_actions</span> Pending</div><span class="rsc-dot" style="background:#f59e0b;"></span></div>
<div class="rsc-number"><?php echo $bookingReports['pending']; ?></div>
<div class="rsc-badge yellow"><span class="material-icons-outlined" style="font-size:12px;">warning</span>Needs action</div>
</div>
<div class="rsc" style="border-top:3px solid #10b981;background:#f5fdf9;">
<div class="rsc-header"><div class="rsc-label"><span class="material-icons-outlined">check_circle</span> Confirmed</div><span class="rsc-dot" style="background:#10b981;"></span></div>
<div class="rsc-number"><?php echo $bookingReports['confirmed']; ?></div>
<div class="rsc-badge green"><span class="material-icons-outlined" style="font-size:12px;">north_east</span>Confirmed</div>
</div>
<div class="rsc" style="border-top:3px solid #ef4444;background:#fff5f5;">
<div class="rsc-header"><div class="rsc-label"><span class="material-icons-outlined">cancel</span> Cancelled</div><span class="rsc-dot" style="background:#ef4444;"></span></div>
<div class="rsc-number"><?php echo $bookingReports['cancelled']; ?></div>
<div class="rsc-badge red"><span class="material-icons-outlined" style="font-size:12px;">south_east</span>Cancelled</div>
</div>
<div class="rsc" style="border-top:3px solid #667eea;background:#fafbff;">
<div class="rsc-header"><div class="rsc-label"><span class="material-icons-outlined">payments</span> Revenue/Month</div><span class="rsc-dot" style="background:#667eea;"></span></div>
<div class="rsc-number">₱<?php echo number_format($bookingReports['revenueThisMonth'],0); ?></div>
...
<!-- Bookings Table with IMPROVED SPACING -->
...
<tr>
<td style="font-weight: 600; color: #4b5563;">#<?php echo str_pad($booking['id'], 6, '0', STR_PAD_LEFT); ?></td>
<td>
<div class="user-info">
...
<td style="font-weight: 500;"><?php echo htmlspecialchars($booking['tour_name']); ?></td>
<td><?php echo date('M j, Y', strtotime($booking['booking_date'])); ?></td>
<td style="text-align: center;"><?php echo $booking['number_of_people']; ?></td>
...
<div class="report-table-card-header"><span class="material-icons-outlined">receipt_long</span><h3>Recent Bookings</h3></div>
<table><thead><tr><th>User</th><th>Tour</th><th>Amount</th><th>Status</th></tr></thead><tbody>
<?php foreach ($bookingReports['recent'] as $rb): ?>
<tr>
<td>
<div style="display: flex; align-items: center; gap: 10px;">
<div style="width: 34px; height: 34px; border-radius: 50%; background: linear-gradient(135deg,#667eea,#764ba2); color: white; display: flex; align-items: center; justify-content: center; font-weight: 600; font-size: 13px; flex-shrink: 0;">
<?php echo strtoupper(substr($rb['user_name'], 0, 1)); ?>
</div>
<span style="font-weight: 600; color: #111827; font-size: 0.85rem;"><?php echo htmlspecialchars($rb['user_name']); ?></span>
</div>
</td>
<td style="font-size:.78rem;color:#6b7280;"><?php echo htmlspecialchars(substr($rb['tour_name'],0,28)); ?><?php echo strlen($rb['tour_name'])>28?'…':''; ?></td>
<td style="font-weight:700;color:#667eea;">₱<?php echo number_format($rb['total_amount'],0); ?></td>
<td><span class="status-badge status-<?php echo $rb['status']; ?>"><?php echo ucfirst($rb['status']); ?></span></td>
</tr>
<?php endforeach; ?>
<?php if (empty($bookingReports['recent'])): ?><tr><td colspan="4" style="text-align:center;color:#9ca3af;padding:24px;">No bookings yet.</td></tr><?php endif; ?>
</tbody></table>
</div>
<div class="report-table-card">
<div class="report-table-card-header"><span class="material-icons-outlined">emoji_events</span><h3>Top Tours by Bookings</h3></div>
<table><thead><tr><th>Rank</th><th>Tour Name</th><th>Bookings</th><th>Revenue</th></tr></thead><tbody>
<?php foreach ($bookingReports['topTours'] as $i => $tt): ?>
<tr>
...
<?php endforeach; ?>
<?php if (empty($bookingReports['topTours'])): ?><tr><td colspan="4" style="text-align:center;color:#9ca3af;padding:24px;">No booking data yet.</td></tr><?php endif; ?>
</tbody></table>
</div>
</div>
</div>
...
function deleteBooking(bookingId) {
    ...
    .then(result => {
        if (!result.success) { showToast(result.message||'Failed to load','error'); return; }
        const b = result.data;
        $id('deleteBookingLabel').textContent = `Booking #${String(bookingId).padStart(6,'0')} (${b.tour_name})`;
        
        // Display booking details for identification
        const details = `
            <div style="display:grid;gap:8px;">
                <div><strong>Booking ID:</strong> #${String(bookingId).padStart(6,'0')}</div>
                <div><strong>Tour:</strong> ${b.tour_name || 'N/A'}</div>
                <div><strong>User:</strong> ${b.user_name || 'N/A'}</div>
                <div><strong>Email:</strong> ${b.user_email || 'N/A'}</div>
                <div><strong>Date:</strong> ${b.booking_date ? date('M j, Y', strtotime(b.booking_date)) : 'N/A'}</div>
                <div><strong>People:</strong> ${b.number_of_people || 'N/A'}</div>
                <div><strong>Amount:</strong> ₱${number_format(b.total_amount || 0, 2)}</div>
                <div><strong>Status:</strong> ${ucfirst(b.status || 'N/A')}</div>
            </div>
        `;
