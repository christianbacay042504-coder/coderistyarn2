...
if ($conn) {
    ...
    // Monthly revenue - calculate from confirmed bookings like Analytics page
    $result = $conn->query("SELECT SUM(total_amount) as total FROM bookings WHERE status = 'confirmed' AND MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE())");
    $monthlyRevenue = $result ? ($result->fetch_assoc()['total'] ?? 0) : 0;
